...
function distributeBaMen(zhiFuGong, shiGan, diPan, type = 'yang') {
    ...
    // 计算转动步数（从值符宫到时干落宫，八门转动步数是九星的2倍）
    const baseSteps = (luoGongIndex - zhiFuIndex + 8) % 8;
    const steps = (baseSteps * 2) % 8;

    // 八门整体转动
    let zhiShiLuoGong = ''; // 值使门转动后所在的宫位
    for (let i = 0; i < 8; i++) {
        ...
        // 记录值使门转动后的位置
        if (originalMen === zhiShiMen) {
            zhiShiLuoGong = newGong;
        }
    }

    return {
        zhiShiGong: zhiShiLuoGong || luoGong,  // 值使门转动后的宫位
        zhiShiMen,
        baMen
    };
}
