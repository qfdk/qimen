...
function calculate(date, options = {}) {
    ...
    try {
        ...
        // 排布八门（使用地盘干找时干落宫）
        const baMenResult = baMenModule.distributeBaMen(jiuXingResult.zhiFuGong, luoGongGan, diPan);

        // 排布八神（阳遁顺时针，阴遁逆时针）
        const baShen = baShenModule.distributeBaShen(jiuXingResult.zhiFuGong, juShu.type);
